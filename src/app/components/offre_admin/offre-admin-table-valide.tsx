...
export function OffreAdminTableValide({
  refresh,
  selectMode = false,
  selectedOffers = [],
  ...
  const selectAllOffers = () => {
    ...
    // Mettre à jour directement via la fonction parent
  ...
  if (offres.length === 0)
    return (
      <div className="p-6 bg-blue-50 border border-blue-200 rounded-lg text-blue-600 flex items-center justify-center">
        ...
        Aucune offre validée disponible
      </div>
    )
  ...
  return (
    <>
      ...
      {/* Vous devrez implémenter ce composant ou l'importer */}
      {selectedOffre && (
        <Dialog open={isEditOpen} onOpenChange={() => setIsEditOpen(false)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Prolonger l'offre</DialogTitle>
            </DialogHeader>
            <DialogDescription>Vous pouvez prolonger la date d'expiration de cette offre.</DialogDescription>
            <DialogFooter>
