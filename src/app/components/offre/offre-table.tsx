...
export function OffreTable({ refresh }: { refresh: boolean }) {
  ...
  useEffect(() => {
    fetchOffres()
  }, [router, refresh]) // Rafraîchir les données lorsque `refresh` change
  ...
  return (
    <>
        ...
        <TableBody>
          {offres.map((offre) => (
            <TableRow key={offre.id}>
              ...
              <TableCell className="max-w-xs truncate">{offre.description}</TableCell>
              ...
              <TableCell>
                <span
                  className={`px-2 py-1 rounded-full text-sm ${offre.valider ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}
                >
      ...
      <OffreEditDialog
        ...
        onOffreUpdated={fetchOffres} // Rafraîchir les offres après une modification
      />
    </>
  )
}